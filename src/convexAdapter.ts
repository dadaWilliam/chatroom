...
// Fix for the Authenticator type - explicitly defining transports as string | undefined
...
export const ConvexAdapter: Adapter = {
  async createAuthenticator(authenticator: AdapterAuthenticator & { userId: Id<"users"> }) {
    // Handle possible null value in transports by converting to undefined
    const processedAuthenticator: Authenticator = {
      ...authenticator,
      transports: authenticator.transports === null ? undefined : authenticator.transports
    };
    
    await callMutation(api.authAdapter.createAuthenticator, { authenticator: processedAuthenticator });
